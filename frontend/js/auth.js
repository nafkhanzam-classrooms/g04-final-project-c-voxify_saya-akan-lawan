export class AuthModule {
  ...
  initNetwork() {
    this.app.network.registerHandler("auth_login", (response) => {
    ...
    this.app.network.registerHandler("auth_register", (response) => {
      if (response.status === "success") {
  ...
  handleLogin() {
    ...
    this.app.network.sendPacket("auth_login", {
      username: username,
      password: password,
    });
  }
  ...
  handleRegister() {
    ...
    if (!username || !email || !display || !password) return;

    this.app.network.sendPacket("auth_register", {
      username: username,
      email: email,
